-- apply_slot_rule_change (20260711_apply_slot_rule_change.sql) n'avait
-- jusqu'ici AUCUNE vérification d'identité : appelable par quiconque connaît
-- p_space_id, y compris un client anonyme hors app. Trouvé en concevant le
-- chemin d'écriture co-admin pour les règles de visite — corrigé ici pour
-- les deux appelants possibles :
--   - admin réel authentifié : auth.uid() est disponible automatiquement
--     dans une fonction security definer appelée par un client authentifié,
--     donc AUCUN changement requis côté client (settings.tsx continue son
--     appel existant tel quel) ;
--   - co-admin (client anonyme, PIN visiteur) : nouveaux paramètres optionnels
--     p_caller_prenom/p_caller_nom/p_caller_pin, vérifiés contre
--     patient_space_coadmins (ligne active+acceptée) puis contre le
--     visitor_profiles précis référencé par cette ligne.
--
-- Le corps de la fonction (recasage des réservations, gestion des nuitées,
-- etc.) est repris à l'identique de 20260711_apply_slot_rule_change.sql —
-- seule la garde d'autorisation en tête est nouvelle.
create or replace function public.apply_slot_rule_change(
  p_space_id uuid,
  p_new_config jsonb,
  p_new_slots text[],
  p_caller_prenom text default null,
  p_caller_nom text default null,
