-- Mode "1 visite / jour" (Règles de visite > Choisir 1 visite par jour) :
-- quand activé pour un espace, une fois qu'un créneau "Visite" est réservé
-- un jour donné, tous les autres créneaux de ce jour deviennent indisponibles
-- pour tout le monde — sauf pour l'auteur de la réservation qui peut encore
-- la déplacer vers un autre créneau du même jour (cf. app/(visitor)/home/slots.tsx
-- et le sélecteur de créneau interne à la modale d'édition de BookingFlow.tsx,
-- laissé inchangé). N'affecte jamais les "Nuit" (écran séparé) ni les
-- "Intervention" (jamais soumises à check_slot_capacity, voir la garde
-- `if new.type <> 'Visite'` plus bas).

alter table public.slot_config
  add column if not exists one_visit_per_day boolean not null default false;

alter table public.slot_config_history
  add column if not exists one_visit_per_day boolean not null default false;

-- 1. check_slot_capacity() : rejette au niveau serveur toute réservation
-- "Visite" sur un créneau différent d'un créneau déjà pris le même jour,
-- en excluant le propre cohort (group_id) de la réservation en cours
-- d'insertion/modification.
...
-- 2. apply_slot_rule_change() : fait transiter one_visit_per_day comme les
-- autres champs de slot_config (config live + historique versionné). Ne
-- déclenche volontairement pas de recasage automatique des réservations
-- existantes : activer le mode ne touche que les réservations futures,
-- les cohorts déjà multiples un même jour restent en l'état (pas de
-- suppression/déplacement rétroactif).
