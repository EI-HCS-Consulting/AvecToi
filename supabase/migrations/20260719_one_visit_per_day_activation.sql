-- Corrige/complète le mode "1 visite / jour" (20260718_one_visit_per_day.sql) :
--
-- 1. Nouvel alert_type/change_type 'day_cap_suspended', pour signaler une
--    réservation "Visite" mise en veille par l'activation du mode (elle
--    n'est ni déplacée ni supprimée — cf. previous_date/previous_creneau
--    inchangés, seul le champ alert_* change).
-- 2. check_slot_capacity() : une réservation déjà "day_cap_suspended" ne
--    doit plus jamais compter comme "le créneau du jour" pris — sinon elle
--    resterait bloquante indéfiniment même une fois son propriétaire averti.
-- 3. apply_slot_rule_change() : au moment où one_visit_per_day passe de
--    false à true (activation), balaie les réservations "Visite" futures
--    (>= aujourd'hui) et, pour chaque jour ayant plusieurs réservations
--    (cohorts) déjà prises, garde active celle enregistrée en premier
--    (created_at le plus ancien) et suspend toutes les autres. Aucune
--    réservation n'est déplacée ni effacée — seules les alert_* sont
--    posées, exactement comme "night_cancelled" : le visiteur voit le
--    popup (RebookingAlertModal) et choisit lui-même un autre jour via
--    "Modifier".

do $$
begin
  alter table public.reservations drop constraint if exists reservations_alert_type_check;
  alter table public.reservations
    add constraint reservations_alert_type_check
    check (alert_type in ('rebooked', 'night_cancelled', 'rebooking_failed', 'day_cap_suspended'));
end $$;

do $$
begin
  alter table public.reservation_change_history drop constraint if exists reservation_change_history_change_type_check;
  alter table public.reservation_change_history
    add constraint reservation_change_history_change_type_check
    check (change_type in ('rebooked', 'night_cancelled', 'rebooking_failed', 'day_cap_suspended'));
end $$;

-- 2. check_slot_capacity()
...
begin
  ...
  -- (date >= current_date, comme le recasage ci-dessus), et ne déplace ni
  -- ne supprime rien — pour chaque jour où plusieurs réservations "Visite"
  -- existent déjà, la première enregistrée (created_at le plus ancien)
  -- reste active, toutes les autres sont marquées "day_cap_suspended"
  -- (alert_* uniquement, cf. le visiteur peut la "Modifier" vers un autre
  -- jour depuis le popup RebookingAlertModal).
